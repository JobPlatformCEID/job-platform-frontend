...
class CvBuilderScreen extends StatefulWidget {
  final Server server;
  final Auth auth;
  const CvBuilderScreen({Key? key, required this.server, required this.auth})
    : super(key: key);
...
class _CvBuilderScreenState extends State<CvBuilderScreen> {
                  ...
                  children: [
                    pw.Row(
                      ...
                      children: [
                        ...
                        pw.Text(
                          '${edu.startDate} - ${edu.endDate.isEmpty ? 'Present' : edu.endDate}',
                          style: pw.TextStyle(
                            color: PdfColors.grey600,
                            fontSize: 8,
                          ),
      ...
      child: pw.Column(
        ...
        children: [
          ...
          if (cv.summary.isNotEmpty) ...[
            ...
            pw.Text(
              cv.summary,
              style: pw.TextStyle(fontSize: 9, lineSpacing: 4),
            ),
            pw.SizedBox(height: 12),
          ],
          if (cv.experience.isNotEmpty) ...[
  ...
  void _openPreview() {
    Navigator.of(
      context,
    ).push(MaterialPageRoute(builder: (_) => _FullScreenPreview(cv: _cv)));
  }
  ...
  Widget _stepTemplate() {
    final templates = [
      (
        CvTemplate.classic,
        'Classic',
        'Traditional layout with clear sections and a clean header.',
      ),
      (
        CvTemplate.modern,
        'Modern',
        'Two-column design with an accent sidebar for a bold look.',
      ),
      (
        CvTemplate.minimal,
        'Minimal',
        'Typography-first, centered header, understated and elegant.',
      ),
    ];
    return ListView(
              ...
              child: Row(
                children: [
                  Radio<CvTemplate>(
                    ...
                    onChanged: (v) =>
                        setState(() => _cv = _cv.copyWith(template: v)),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Column(
    ...
    return ListView(
      ...
      children: [
        ...
        _field(
          'Full Name',
          _cv.fullName,
          (v) => _cv = _cv.copyWith(fullName: v),
          validator: (v) => v!.isEmpty ? 'Required' : null,
        ),
        _field(
          'Job Title',
          _cv.jobTitle,
          (v) => _cv = _cv.copyWith(jobTitle: v),
          hint: 'e.g. Marketing Manager, Teacher, Designer',
        ),
        _field(
          'Email',
          _cv.email,
          (v) => _cv = _cv.copyWith(email: v),
          validator: (v) =>
        ...
        _field(
          'Location',
          _cv.location,
          (v) => _cv = _cv.copyWith(location: v),
          hint: 'City, Country',
        ),
        _field(
          'LinkedIn',
          _cv.linkedin,
          (v) => _cv = _cv.copyWith(linkedin: v),
          hint: 'linkedin.com/in/yourname',
        ),
        _field(
          'Website / Portfolio',
          _cv.website,
          (v) => _cv = _cv.copyWith(website: v),
  ...
  Widget _stepExperience() {
    return ListView(
      ...
      children: [
        ...
        ..._cv.experience.asMap().entries.map(
          (e) => _experienceCard(
            e.value,
            (updated) {
  ...
  Widget _stepEducation() {
    return ListView(
      ...
      children: [
        ...
        ..._cv.education.asMap().entries.map(
          (e) => _educationCard(
            e.value,
            (updated) {
      ...
      builder: (context, setLocal) {
        return ListView(
          ...
          children: [
            _stepTitle('Skills'),
            const Text(
              'Add each skill individually and press Add.',
              style: TextStyle(fontSize: 12, color: Color(0xFF666666)),
            ),
            const SizedBox(height: 16),
            Row(
            ...
            if (_cv.skills.isNotEmpty)
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: _cv.skills.asMap().entries.map((e) {
                  return Chip(
                    label: Text(e.value),
                    backgroundColor: const Color.fromARGB(166, 0, 212, 184),
                    labelStyle: const TextStyle(color: Color(0xFF212121)),
                    deleteIcon: const Icon(Icons.close, size: 16),
                    onDeleted: () {
                      final list = List<String>.from(_cv.skills);
                      list.removeAt(e.key);
                      setState(() => _cv = _cv.copyWith(skills: list));
                      setLocal(() {});
                    },
                  );
                }).toList(),
  ...
  Widget _stepExtra() {
    return ListView(
      ...
      children: [
        ...
        ..._cv.certifications.asMap().entries.map(
          (e) => _certCard(
            e.value,
            (updated) {
  ...
  Widget _experienceCard(
    CvExperience exp,
    Function(CvExperience) onUpdate,
        ...
        child: Column(
          ...
          children: [
            ...
            _field(
              ...
              exp.company,
              (v) => onUpdate(exp.copyWith(company: v)),
            ),
            _field(
              'Position / Role',
              exp.position,
              (v) => onUpdate(exp.copyWith(position: v)),
            ),
            Row(
              children: [
                Expanded(
                  child: _field(
                    'Start Date',
                    exp.startDate,
                    (v) => onUpdate(exp.copyWith(startDate: v)),
                  ),
                ),
                ...
                Expanded(
                  child: _field(
                    'End Date',
                    exp.endDate,
                    (v) => onUpdate(exp.copyWith(endDate: v)),
            ...
            _textArea(
              'Description',
              exp.description,
              (v) => onUpdate(exp.copyWith(description: v)),
            ),
          ],
        ),
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            _field(
              'Institution',
              edu.institution,
              (v) => onUpdate(edu.copyWith(institution: v)),
            ),
            Row(
              children: [
                Expanded(
                  child: _field(
                    'Degree / Award',
                    edu.degree,
                    (v) => onUpdate(edu.copyWith(degree: v)),
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: _field(
                    'Subject / Field',
                    edu.field,
                    (v) => onUpdate(edu.copyWith(field: v)),
                  ),
                ),
              ],
            ),
              ...
              children: [
                Expanded(
                  child: _field(
                    'Start Date',
                    edu.startDate,
                    (v) => onUpdate(edu.copyWith(startDate: v)),
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: _field(
                    'End Date',
                    edu.endDate,
                    (v) => onUpdate(edu.copyWith(endDate: v)),
                    hint: 'Leave blank if current',
                  ),
                ),
              ],
            ...
            _textArea(
              'Notes',
              edu.description,
              (v) => onUpdate(edu.copyWith(description: v)),
            ),
          ],
        ),
      ),
  ...
  Widget _certCard(
    CvCertification cert,
    Function(CvCertification) onUpdate,
    VoidCallback onDelete,
  ) {
    return Card(
      margin: const EdgeInsets.only(bottom: 12),
        ...
        child: Column(
          ...
          children: [
            ...
            _field(
              'Certificate Name',
              cert.name,
              (v) => onUpdate(cert.copyWith(name: v)),
            ),
            Row(
              children: [
                Expanded(
                  child: _field(
                    'Issuing Body',
                    cert.issuer,
                    (v) => onUpdate(cert.copyWith(issuer: v)),
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: _field(
                    'Date',
                    cert.date,
                    (v) => onUpdate(cert.copyWith(date: v)),
                  ),
                ),
              ],
            ),
